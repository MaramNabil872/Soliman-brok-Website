...
const ServiceItem = ({ title, description, imageSource }) => {
  ...
  return (
    <div
      className={`col-12 col-lg-2 col-md-4 card m-4 bg-white service rounded-lg text-center ${
        isHovered ? "hover" : ""
      }`}
      onMouseOver={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      <img className="w-50 text-center" src={imageSource} />
      <h5 id="service-title" className="text-start pt-2">
        {title}
      </h5>
      <small className="text-muted text-center">{description}</small>
    </div>
  );
};
